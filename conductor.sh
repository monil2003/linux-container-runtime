...
# }
get_layer_hash() {
    local layer=$(echo "$1" | awk -F: '{print $NF}')
    local name=$(basename "$layer")

    if [ "$name" = "diff" ]; then
        dirname "$layer" | xargs basename
    else
        basename "$layer"
    fi
}
...
run() {
    ...
    # unshare --mount --uts --ipc --net --pid --fork --kill-child chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "mount -t proc proc /proc && mount -t sysfs sys /sys && exec $INIT_CMD_ARGS"
...
stop() {
    ...
    umount $MERGED || die "Failed to unmount $MERGED"
